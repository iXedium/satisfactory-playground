...
export const getItemById = async (id) => await db.items.get(id);
// Retrieve a recipe by its ID.
export const getRecipeById = async (id) => await db.recipes.get(id);
// Retrieve recipes that produce a specific item.
export const getRecipesForItem = async (itemId) => {
    ...
    try {
        ...
        if (recipes.length === 0) {
            ...
            const matchingRecipes = allRecipes.filter(recipe => recipe.out && Object.keys(recipe.out).includes(itemId));
...
export const getRecipeByOutput = async (itemId) => {
    return await db.getRecipeByOutput(itemId);
};
// Retrieve machine data for a recipe
export const getMachineForRecipe = async (recipeId) => {
    try {
        ...
        try {
            ...
            const machineData = data.items.find((item) => item.id === producerId && item.machine);
...
export const getIconForItem = async (itemId) => await db.icons.get(itemId);
