...
- [ ] **Task 2: Generalize Node Creation Logic** (In Progress)
  ...
  - **Details:** Refactored `calculateAndAutoImportThunk` to remove complex internal state management. It now calculates the base tree, dispatches `setDependencies`, and triggers `autoImportNodeChildrenThunk` and `recalculateAndUpdateRootAmountThunk`. *(Still observing potential state reset issues during creation)*.

- [X] **Task 3: Generalize Node Deletion Logic** (Completed)
  ...
  - **Files Affected:** `importExportLogic.ts`, `usePlannerDataManagement.ts`
  - **Details:** Confirmed that `handleDeleteTree` in `usePlannerDataManagement.ts` calls `destroyNodeRecursiveThunk`. Confirmed `destroyNodeRecursiveThunk` correctly dispatches `removeNodeAction` and triggers `requestDependencyCheckThunk`.

- [X] **Task 4: Generalize Node Type Conversion (N <-> B)** (Completed - No Changes)
  - **Goal:** Refine `checkAndConvertNodeTypeThunk` to be the sole handler for type conversions, potentially simplifying its internal logic by calling other generalized functions.
  - **Files Affected:** `importExportLogic.ts`
  - **Details:** Analyzed `checkAndConvertNodeTypeThunk`. 
    - N->B conversion already seems streamlined and uses `requestDependencyCheckThunk` appropriately.
    - B->N conversion requires explicit child calculation/update before calling `autoImportNodeChildrenThunk` due to the latter's design. No simplification identified without larger architectural changes.
